...
Discovery keeps three decisions distinct: (1) is this a valid signed Pontmore agent definition, (2) does it describe a provider compatible with the requested capability, and (3) does the requester's deterministic economic policy authorize the provider's current signed offer. Malformed candidates are rejected independently with structured rejection categories without aborting the whole query. Query sizes and follow-up resolutions are bounded. The deterministic ordering is: authorized offers only, lowest price, shortest declared maximum execution time, then provider pubkey ascending as the final stable tie-break. An optional advisory preference may break ties only among already-authorized candidates and can never authorize a rejected offer; AI failure or absence never widens authority.
